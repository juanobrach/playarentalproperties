...
$list_of_relies= '<div class="mess_content-list-replies">';
    ...
    while ($message_selection_child->have_posts()): $message_selection_child->the_post(); 
        ...
        $list_of_relies.= '<div class="mess_content-list-replies_unit" '.$post->ID.' data-mess-reply_user_id="'.$mes_to.'" data-mess-reply_user_froom="'.$mess_from.'" >';
        $list_of_relies.= '<h4><strong>'.esc_html__( 'From: ','wprentals').'</strong> '.$user->user_login.' - ' .get_the_title($post->ID).'</h4>';
...
<div class="col-md-12"> 
    <div class="message_listing " data-messid="<?php print $original_mess ?> "  >

        <div class="message_header">
            <div class="col-md-4">
                <?php
                if($unread_replies>0){
                    print '<span class="mess_unread mess_tooltip" data-original-title="'. esc_html__( 'new message','wprentals').'"><i class="fas fa-exclamation-circle"></i></span>';        
                }
                ...
                if($current_user->user_login == $message_from_user_name ){
                    ...
                    print ' <span class="mess_from"><strong>'.esc_html__( 'From','wprentals'). ': </strong>' .$message_from_user_name. '</span>';       
                }
            ...
            <div class="col-md-4">
                <span class="mess_subject"> <strong><?php esc_html_e('Subject','wprentals');?>: </strong><?php print $message_title;?></span>
            </div>
            ...
            <div class=" message-action text-right" >
                <span data-original-title="<?php esc_html_e('reply to message','wprentals');?>"  class="mess_reply mess_tooltip">       
                    <i class="fas fa-reply"></i> 
                </span>
                <div class="delete_wrapper">
                    <span data-original-title="<?php esc_html_e('delete message','wprentals');?>"  class="mess_delete mess_tooltip">
        ...
        <div class="mess_reply_form"   <?php echo get_post_meta($original_mess, 'delete_destination'.$reply_to_id, true).' cccc '.get_post_meta($original_mess, 'delete_source'.$reply_to_id, true).'fff';?> data-unread_replies="<?php print $unread_replies;?>" >
          
                <h4><?php esc_html_e('Reply','wprentals');?></h4>
                <input type="text" class="subject_reply" value="Re: <?php print $message_title; ?>">
                ...
                <span class="wpb_btn-info wpb_btn-small wpestate_vc_button  vc_button mess_send_reply_button"
                    data-mess-reply_to_id="<?php print $reply_to_id;?>" 
                    data-unread_replies="<?php print $unread_replies;?>" >
