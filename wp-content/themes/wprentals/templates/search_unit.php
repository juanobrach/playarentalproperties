...
<div class="search_unit_wrapper">
    <h4> <?php the_title(); ?> </h4>
    <a class="delete_search" data-searchid="<?php print $post->ID; ?>"><?php esc_html_e('delete search','wprentals');?></a>
    <?php  
    ...
    foreach($search_arguments_decoded->tax_query as $key=>$query ){
        ...
        if ( isset($query->taxonomy) && isset($query->terms[0]) && $query->taxonomy=='property_category'){
            ...
            if( !empty($page) ){
                print '<strong>'.esc_html__( 'Category','wprentals').':</strong> '. $page->name .', ';  
            }
        }
        ...
        if ( isset($query->taxonomy) && isset($query->terms[0]) && $query->taxonomy=='property_action_category'){
           $page = get_term_by( 'slug',$query->terms[0] ,'property_action_category');
            if( !empty($page) ){
                print '<strong>'.esc_html__( 'For','wprentals').':</strong> '.$page->name.', ';  
            }
        ...
        if ( isset($query->taxonomy) && isset($query->terms[0]) && $query->taxonomy=='property_city'){
            ...
            if( !empty($page) ){
                print '<strong>'.esc_html__( 'City','wprentals').':</strong> '.$page->name.', ';  
            }
        ...
        if ( isset($query->taxonomy) && isset($query->terms[0]) && $query->taxonomy=='property_area'){
            ...
            if( !empty($page) ){
                print '<strong>'.esc_html__( 'Area','wprentals').':</strong> '.$page->name.', ';  
            }
        ...
        if($custom_advanced_search==='yes'){
            ...
            if ( isset($query->compare) ){
                ...
                if ($query->compare=='CHAR'){
                    ...
                    print '<strong>'.$custm_name.'</strong> '.esc_html__( 'smaller than ','wprentals').' '.$query->value.', ';            
                }  else{
                    print '<strong>'.$custm_name.'</strong> '.esc_html__( 'bigger than','wprentals').' '.$query->value.', ';   
                }                
            }else{
            ...
            if ( isset($query->compare) ){
                if ($query->compare=='CHAR'){
                    ...
                    print '<strong>'.str_replace('_',' ',$query->key).'</strong> '.esc_html__( 'smaller than ','wprentals').' '.$query->value.', ';            
                } else{
                     print '<strong>'.str_replace('_',' ',$query->key).'</strong> '.esc_html__( 'bigger than ','wprentals').' '.$query->value.', ';            
                }                 
            }else{
