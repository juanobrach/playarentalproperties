...
class Wpestate_footer_latest_widget extends WP_Widget {
	...
	function form($instance){
		...
		$instance = wp_parse_args((array) $instance, $defaults);
                ...
                $all_selected='';
                if(!empty($current_adv_filter_search_action) &&  in_array  (esc_html__( 'all','wprentals'),$current_adv_filter_search_action)  ){
		...
		print $before_widget;
			...
			print $before_title.$title.$after_title;
                ...
                if( $output === false){
                    ...
                    ob_end_clean();
                    set_transient($transient_name,$output,4*60*60);
                    wp_reset_query();
                }
                print $output;
		$display.='</div>';
