...
if( !function_exists('wpestate_create_membership_type') ):
    function wpestate_create_membership_type() {
        register_post_type( 'membership_package',
                    array(
                            'labels' => array(
                                    'name'          => esc_html__(  'Membership Packages','wprentals'),
                                    'singular_name' => esc_html__(  'Membership Packages','wprentals'),
                                    'add_new'       => esc_html__( 'Add New Membership Package','wprentals'),
                    'add_new_item'          =>  esc_html__( 'Add Membership Packages','wprentals'),
                    'edit'                  =>  esc_html__( 'Edit Membership Packages' ,'wprentals'),
                    'edit_item'             =>  esc_html__( 'Edit Membership Package','wprentals'),
                    'new_item'              =>  esc_html__( 'New Membership Packages','wprentals'),
                    'view'                  =>  esc_html__( 'View Membership Packages','wprentals'),
                    'view_item'             =>  esc_html__( 'View Membership Packages','wprentals'),
                    'search_items'          =>  esc_html__( 'Search Membership Packages','wprentals'),
                    'not_found'             =>  esc_html__( 'No Membership Packages found','wprentals'),
                    'not_found_in_trash'    =>  esc_html__( 'No Membership Packages found','wprentals'),
                    'parent'                =>  esc_html__( 'Parent Membership Package','wprentals')
                            ),
                    'public' => false,
                    'show_ui'=>true,
...
if( !function_exists('wpestate_add_pack_metaboxes') ):
    function wpestate_add_pack_metaboxes() {	
        add_meta_box(  'estate_membership-sectionid',  esc_html__(  'Package Details', 'wprentals' ),'membership_package','membership_package' ,'normal','default'
    );
}
    ...
    function membership_package( $post ) {
	    ...
	    global $post;
                ...
                <p class="meta-options">
                    <label for="biling_period">'.esc_html__( 'Billing Time Unit :','wprentals').'</label><br />
                    '.$billing_select.'
                </p>
                
                <p class="meta-options">
                    <label for="billing_freq">'.esc_html__( 'Bill every x units','wprentals').' </label><br />
                    <input type="text" id="billing_freq" size="58" name="billing_freq" value="'.  intval(get_post_meta($post->ID, 'billing_freq', true)).'">
                </p>
                
                <p class="meta-options">
                    <label for="pack_listings">'.esc_html__( 'How many listings are included?','wprentals').'</label><br />
                    ...
                    <label for="mem_list_unl">'.esc_html__( 'Unlimited listings ?','wprentals').'</label>
                </p>
                
                <p class="meta-options">
                    <label for="pack_featured_listings">'.esc_html__( 'How many Featured listings are included?','wprentals').'</label><br />
                ...
                <p class="meta-options">
                    <label for="pack_price">'.esc_html__( 'Package Price in ','wprentals'). ' ' . wpestate_curency_submission_pick().'</label><br />
                    <input type="text" id="pack_price" size="58" name="pack_price" value="'.  esc_html(get_post_meta($post->ID, 'pack_price', true)).'">
		</p>

                <p class="meta-options">
                    <label for="pack_visible">'.esc_html__( 'Is visible? ','wprentals').'</label><br />
                    '.$visible_select.'
		</p>
...
if( !function_exists('wpestate_get_pack_data_for_user_top') ):
    function wpestate_get_pack_data_for_user_top($userID,$user_pack,$user_registered,$user_package_activation){
            ...
            if($remaining_lists==-1){
                $remaining_lists=esc_html__('unlimited','wprentals');
            }
            ...
            if ($user_pack!=''){
               ...
               $expired_date    =   date('Y-m-d',$expired_date); 
                ...
                print '<div class="pack-name">'.$title.' <span>'.esc_html__( 'Current Subscription','wprentals').'</span></div>';
                $extra_pack_class='';
                if($pack_list>999){
                ...
                print '<div class="pack-info '.$extra_pack_class.'">';
                 ...
                 if($unlimited_lists==1){
                    print '<div class="normal_list_no">  &#8734 <span>'.esc_html__( 'Listings Included','wprentals').'</span></div>';
                    print '<div class="normal_list_no">  &#8734 <span>'.esc_html__( 'Remaining Listings','wprentals').'</span></div>';
                }else{
                    print '<div class="normal_list_no">'.$pack_list      .'<span>'.esc_html__( 'Listings Included','wprentals').'</span></div>';
                    print '<div class="normal_list_no">'.$remaining_lists.'<span>'.esc_html__( 'Remaining Listings','wprentals').'</span></div>';
                
                }
            
                print '<div class="normal_list_no">'.$pack_featured.'<span>'.esc_html__( 'Featured included','wprentals').'</span></div>';
                print '<div class="normal_list_no extend_normal_list_no">'.wpestate_get_remain_featured_listing_user($userID).'<span>'.esc_html__( 'Featured remaining','wprentals').'</span></div>';
                print '<div class="pack-date-wrapper">'.esc_html__( 'Expiration date','wprentals').': <span class="pack-date-wrapper-date">'.$expired_date.'</span></div></div>';
...
///////////////////////////
                ...
                print '<div class="pack-name"  >'.esc_html__( 'Free Membership','wprentals').'<span>'.esc_html__( 'Current Subscription','wprentals').'</span></div>';
                
              
                print '<div class="pack-info '.$extra_pack_class.'"><div class="normal_list_no"> ';
                 if($free_mem_list_unl==1){
                    print esc_html__( '-','wprentals');
                }else{
                    print $free_mem_list;
                }
                print '<span>'.esc_html__( 'Listings Included','wprentals').'</span></div>';
                print '<div class="normal_list_no">'.$remaining_lists.'<span>'.esc_html__( 'Remaining listings','wprentals').'</span></div>';
                print '<div class="normal_list_no">'.$free_feat_list.'<span>'.esc_html__( 'Featured listings included','wprentals').'</span></div>';
                print '<div class="normal_list_no extend_normal_list_no">'.wpestate_get_remain_featured_listing_user($userID).'<span>'.esc_html__( 'Featured Listings remaining','wprentals').'</span></div>';
                print '<div class="pack-date-wrapper">'.esc_html__( 'Expiration date','wprentals').' -</div></div>';
...
if( !function_exists('wpestate_show_bill_period') ):
    function wpestate_show_bill_period($biling_period){
            ...
            if($biling_period=='Day' || $biling_period=='Days'){
                return  esc_html__( 'days','wprentals');
            }
            else if($biling_period=='Week' || $biling_period=='Weeks'){
               return  esc_html__( 'weeks','wprentals');
            }
            else if($biling_period=='Month' || $biling_period=='Months'){
                return  esc_html__( 'months','wprentals');
            }
            else if($biling_period=='Year'){
                return  esc_html__( 'year','wprentals');
            }
...
function wpestate_get_pack_data_for_user($userID,$user_pack,$user_registered,$user_package_activation){
            ...
            if ($user_pack!=''){
                ...
                print '<strong>'.esc_html__( 'Your Current Package: ','wprentals').'</strong></br><strong>'.$title.'</strong></br> ';
                print '<p class="full_form-nob">';
                if($unlimited_lists==1){
                    print esc_html__( '  Unlimited listings','wprentals');
                }else{
                    print $pack_list.esc_html__( ' Listings','wprentals');
                    print ' - '.wpestate_get_remain_listing_user($userID,$user_pack).esc_html__( ' remaining ','wprentals').'</p>';
                }

                print ' <p class="full_form-nob"> <span id="normal_list_no">'.$pack_featured.esc_html__( ' Featured listings','wprentals').'</span>';
                print ' - <span id="featured_list_no">'.wpestate_get_remain_featured_listing_user($userID).'</span>'.esc_html__( ' remaining','wprentals').' </p>';
                ...
                print '<strong>'.esc_html__( 'Your Current Package: ','wprentals').'</strong></br><strong>'.esc_html__( 'Free Membership','wprentals').'</strong>';
                print '<p class="full_form-nob">';
                if($free_mem_list_unl==1){
                     print esc_html__( 'Unlimited listings','wprentals');
                }else{
                     print $free_mem_list.esc_html__( ' Listings','wprentals');
                     print ' - <span id="normal_list_no">'.wpestate_get_remain_listing_user($userID,$user_pack).'</span>'.esc_html__( ' remaining','wprentals').'</p>';

                }
                print '<p class="full_form-nob">';
                print $free_feat_list.esc_html__( ' Featured listings','wprentals');
                print ' - <span id="featured_list_no">'.wpestate_get_remain_featured_listing_user($userID).'</span>'.esc_html__( '  remaining','wprentals').' </p>';
            }
...
if( !function_exists('wpestate_display_packages') ):
    function wpestate_display_packages(){
        ...
        $return='<select name="pack_select" id="pack_select" class="select-submit2"><option value="">'.esc_html__( 'Select package','wprentals').'</option>';
...
if( !function_exists('wpestate_upgrade_user_membership') ):
    function wpestate_upgrade_user_membership($user_id,$pack_id,$type,$paypal_tax_id){
        ...
        $message  = esc_html__( 'Hi there,','wprentals') . "\r\n\r\n";
        $message .= sprintf( esc_html__( "Your new membership on  %s is activated! You should go check it out.",'wprentals'), get_option('blogname')) . "\r\n\r\n";
...
if ( ! function_exists( 'wpestate_render_single_userid' ) ) {
	...
	function wpestate_render_single_userid( $user_o ) {
		...
		if ( ! empty( $user_o ) ) {
			...
			$verify_label       = ( $user_id_verified == 0 ) ? esc_html__( 'Validate user ID', 'wprentals' ) : esc_html__( 'Remove user ID validation', 'wprentals' );
			...
			if ( 'user-edit' !== get_current_screen()->id ) {
				$verification_users .= sprintf( '<div class="eit-profile"><span class="user-display-name">%3$s</span> <a href="%1$s">%2$s</a></div>', esc_url( get_edit_user_link( $user_o->ID ) ), esc_html__( 'Edit/view user profile', 'wprentals' ), esc_html( $user_o->display_name ) );
			}
...
if ( ! function_exists( 'wpestate_display_userID' ) ) {
	...
	function wpestate_display_userID( $profileuser ) {
		...
		if ( $verification_users ) {
			$verifications .= sprintf( '<h2>%s</h2>', esc_html__( 'User ID', 'wprentals' ) ) . PHP_EOL;
			$verifications .= '<div class="user-verifications">' . PHP_EOL;
			$verifications .= $verification_users;
			$verifications .= '</div> <!-- end .user-verifications -->' . PHP_EOL;
		}
...
if ( ! function_exists( 'wpestate_display_verification_badge' ) ) {
	...
	function wpestate_display_verification_badge( $userID ) {
		...
		if ( $user_verified ) {
			$verified = sprintf('<span class="verified_userid"><i class="fas fa-check-circle" aria-hidden="true"></i> %s</span>', esc_html__('Verified','wprentals'));
		}
		return $verified;
	}
}
