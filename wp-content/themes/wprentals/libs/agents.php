...
if( !function_exists('wpestate_create_agent_type') ):
    function wpestate_create_agent_type() {
        register_post_type( 'estate_agent',
            array(
                'labels' => array(
                'name'                  => esc_html__(  'Owners','wprentals'),
                'singular_name'         => esc_html__(  'Owner','wprentals'),
                'add_new'               => esc_html__( 'Add New Owner','wprentals'),
                'add_new_item'          =>  esc_html__( 'Add Owner','wprentals'),
                'edit'                  =>  esc_html__( 'Edit' ,'wprentals'),
                'edit_item'             =>  esc_html__( 'Edit Owner','wprentals'),
                'new_item'              =>  esc_html__( 'New Owner','wprentals'),
                'view'                  =>  esc_html__( 'View','wprentals'),
                'view_item'             =>  esc_html__( 'View Owner','wprentals'),
                'search_items'          =>  esc_html__( 'Search Owner','wprentals'),
                'not_found'             =>  esc_html__( 'No Owners found','wprentals'),
                'not_found_in_trash'    =>  esc_html__( 'No Owners found','wprentals'),
                'parent'                =>  esc_html__( 'Parent Owner','wprentals')
            ),
            'public' => true,
            'has_archive' => true,
...
if( !function_exists('wpestate_add_agents_metaboxes') ):
    function wpestate_add_agents_metaboxes() {	
        add_meta_box(  'estate_agent-sectionid', esc_html__(  'Owner Details', 'wprentals' ), 'estate_agent', 'estate_agent' ,'normal','default');
    }
...
if( !function_exists('wpsx_5688_update_post') ):
    function wpsx_5688_update_post($post_id,$post){
        ...
        if ( $new_user_id){
         //   esc_html_e('The email was not saved because it is used by another user.</br>','wprentals');
        } else{
            $args = array(
                 'ID'         => $user_id,
                 'user_email' => $email
