...
if( !function_exists('wpestate_ajax_add_booking') ):
    function wpestate_ajax_add_booking(){
      ...
      //  check_ajax_referer( 'booking_ajax_nonce','security');
        ...
        $renter_name            =   $_POST['rentername'];
        $renter_email            =   $_POST['renteremail'];
        $total_value            =   $_POST['totalvalue'];
        $amount_paid            =   $_POST['amountpaid'];
        
        print 'xxx $fromdate'.$fromdate.' / '.$to_date;
        ...
        update_post_meta($post_id, 'am_renter_name', $renter_name);
        update_post_meta($post_id, 'am_renter_email', $renter_name);
        update_post_meta($post_id, 'am_total_value', $total_value);
        update_post_meta($post_id, 'am_already_paid', $amount_paid);
                        ...
                        <div class="invoice_row invoice_content">
                            ...
                            if($booking_array['price_per_guest_from_one']==1){
                                ...
                                if($booking_array['cover_weekend']){
                                    ...
                                    if($booking_array['has_custom']){
                                        ...
                                        $new_price_to_show=$price_show.' '.esc_html__( 'per night','wprentals');
                                    }
                                }
                            ...
                            if($booking_array['has_guest_overload']!=0 && $booking_array['total_extra_price_per_guest']!=0){
                                ...
                                <div class="invoice_row invoice_content">
                                    ...
                                    <span class="inv_exp">   ('.$booking_array['numberDays'].' '.esc_html__( 'nights','wprentals').' | '.$booking_array['extra_guests'].' '.esc_html__('extra guests','wprentals').' ) </span>
                                </div>';
    ...
    if( isset( $_POST['listing_edit'] ) ) {
        if( !is_numeric($_POST['listing_edit'] ) ){
            ...
            ////////////////////////////////////////////////////////////////////    
                ...
                $submission_page_fields         =   get_option('wp_estate_submission_page_fields','') ;
                $mandatory_page_fields          =   get_option('wp_estate_mandatory_page_fields','') ;
                $rental_type                    =   get_option('wp_estate_item_rental_type',true);
