...
if( !function_exists('estate_sidebar_meta') ):
    function estate_sidebar_meta() {
        global $post;
        add_meta_box('wpestate-sidebar-post',       esc_html__( 'Sidebar Settings',  'wprentals'), 'estate_sidebar_box', 'post');
        add_meta_box('wpestate-sidebar-page',       esc_html__( 'Sidebar Settings',  'wprentals'), 'estate_sidebar_box', 'page');
        add_meta_box('wpestate-sidebar-property',   esc_html__( 'Sidebar Settings',  'wprentals'), 'estate_sidebar_box', 'estate_property');
        add_meta_box('wpestate-sidebar-agent',      esc_html__( 'Sidebar Settings',  'wprentals'), 'estate_sidebar_box', 'estate_agent');
        add_meta_box('wpestate-settings-post',      esc_html__( 'Post Settings',     'wprentals'), 'estate_post_options_box', 'post', 'normal', 'default' );
        add_meta_box('wpestate-settings-page',      esc_html__( 'Page Settings',     'wprentals'), 'estate_page_options_box', 'page', 'normal', 'default' );
        if(isset($post->ID)){
            if(  basename(get_page_template($post->ID))== 'property_list.php' || basename(get_page_template($post->ID))== 'property_list_half.php' ){
                add_meta_box('wpestate-pro_list_adv',       esc_html__( 'Property List Advanced Options','wprentals'), 'estate_prop_advanced_function', 'page', 'normal', 'low');
            }
        }
        add_meta_box('wpestate-header',             esc_html__( 'Appearance Options','wprentals'), 'estate_header_function', 'page', 'normal', 'low');
        add_meta_box('wpestate-header',             esc_html__( 'Appearance Options','wprentals'), 'estate_header_function', 'post', 'normal', 'low');
        add_meta_box('wpestate-header',             esc_html__( 'Appearance Options','wprentals'), 'estate_header_function', 'estate_agent', 'normal', 'low');
        //add_meta_box('wpestate-header',             esc_html__( 'Header Options','wprentals'), 'estate_header_function', 'estate_property', 'normal', 'low');
        add_meta_box('wpestate-header', esc_html__('Stars','wprentals'), 'estate_comment_starts', 'comment', 'normal');
    }
...
if( !function_exists('estate_header_function') ):
    function estate_header_function(){
        ...
        print'
            <h3 class="pblankh">'.__('Use transparent header','wprentals').'</h3>
            <select name="transparent_status">
                '.$transparent_symbol.'
            </select>';
        
        print '   
            <h3 class = "pblankh">'.__('Select header type','wprentals').'</h3>
...
function estate_prop_advanced_function(){
    ...
    if( basename(get_page_template($post->ID))!= 'property_list.php' && basename(get_page_template($post->ID))!= 'property_list_half.php' ){
        esc_html_e('Only for "Properties List" page template ! ','wprentals');
        return;
    }
    ...
    $actions_select.='<option value="all" '.$all_selected.'>'.esc_html__( 'all','wprentals').'</option>';
    if( !empty( $tax_terms ) ){                       
    ...
    $categ_select.='<option value="all" '.$all_selected.'>'.esc_html__( 'all','wprentals').'</option>';
    if( !empty( $tax_terms ) ){                       
        foreach ($tax_terms as $tax_term) {
 ...
 //////////////////////////////////////////////////////////////////////////////////////////   
    ...
    $select_city.='<option value="all" '.$all_selected.' >'.esc_html__( 'all','wprentals').'</option>';
 ...
 //////////////////////////////////////////////////////////////////////////////////////////   
    ...
    $select_area.='<option value="all" '.$all_selected.'>'.esc_html__( 'all','wprentals').'</option>';
...
if( !function_exists('estate_listing_options') ):
    function estate_listing_options(){
        ...
        if ( 'property_list.php'== basename( get_page_template() )){
          ...
          print '
          <p class="meta-options">
            <label for="listing_action">'.esc_html__( 'Action category','wprentals').'</label><br />
        ...
        <p class="meta-options">
        <label for="listing_categ">'.esc_html__( 'Pick Category','wprentals').'</label><br />
        ...
        <p class="meta-options">
          <label for="listing_city">'.esc_html__( 'Pick City','wprentals').'</label><br />
          <select  name="listing_city"  >
                <option value="all">'.esc_html__( 'All Cities','wprentals').'</option>
                '. $select_city.'
           </select>
        </p>

        <p class="meta-options">
            <label for="listing_area">'.esc_html__( 'Pick Area','wprentals').'</label><br />
            <select  name="listing_area">
                <option data-parentcity="*" value="all">'.esc_html__( 'All Areas','wprentals').'</option>
                '.$select_area.'
            </select>
        </p>
         ';
            ...
            print esc_html_e('These Options are available for "Property list" page template only!','wprentals');
        }
    ...
    function estate_page_slider_box($post) {
        ...
        <div class="header_admin_options revolution_slider">
            <p class="meta-options pblank">
                <h3 class="pblankh">'.esc_html__( 'Options for Revolution Slider (if Header Type "revolution slider" is selected)','wprentals').'</h3>
            </p>
            <p class="meta-options">	
                <label for="page_custom_lat">'.esc_html__( 'Revolution Slider Name','wprentals').'</label><br />
...
function estate_page_map_box($post) {
    ...
    print ' <div class="header_admin_options google_map">
        <p class="meta-options pblank">
        <h3 class="pblankh">'.esc_html__( 'Options for Google Maps (if Header Type "google map" is selected)','wprentals').'</h3>
        </p>';
    ...
    if( get_post_type($post->ID)!="estate_property" ){
        ...
        <p class="meta-options pblank">
        '.esc_html__( '  Leave these blank in order to get the general map settings.','wprentals').'
        </p>
        <p class="meta-options third-meta-options">	
        <label for="page_custom_lat">'.esc_html__( 'Map - Center point  Latitudine: ','wprentals').'</label><br />
        <input type="text" id="page_custom_lat" name="page_custom_lat" size="36" value="'.$page_lat.'">
        </p>

        <p class="meta-options third-meta-options">	
        <label for="page_custom_long">'.esc_html__( 'Map - Center point  Longitudine: ','wprentals').'</label><br />
    ...
    print'
    <p class="meta-options third-meta-options">
     <label for="min_height">'.esc_html__( 'Height of the map when closed','wprentals').'</label><br />
      <input id="min_height" type="text" size="36" name="min_height" value="'.$min_height.'" />
    </p>

    <p class="meta-options third-meta-options">
       <label for="max_height">'.esc_html__( 'Height of map when open','wprentals').'</label><br />
       <input id="max_height" type="text" size="36" name="max_height" value="'.$max_height.'" />
    </p>

    <p class="meta-options third-meta-options">
       <label for="keep_min">'.esc_html__( 'Force map at the "closed" size ? ','wprentals').'</label><br />
    ...
    <p class="meta-options">
        <label for="bypass_fit_bounds">'.esc_html__( 'ByPass fit bounds (auto zoom and pan of the map around visible markers) ','wprentals').'</label><br />
...
print'<div class="header_admin_options image_header">
    <p class="meta-options pblank">
    <h3 class="pblankh">'.esc_html__( 'Options for Static Image  (if Header Type "image" is selected)','wprentals').'</h3>
    </p>
     
   <p class="meta-options">
        <label for="page_custom_image">'.esc_html__( 'Header Image','wprentals').'</label><br />
        <input id="page_custom_image" type="text" size="36" name="page_custom_image" value="'.$page_custom_image.'" />
	<input id="page_custom_image_button" type="button"   size="40" class="upload_button button" value="'.esc_html__( 'Upload Image','wprentals').'" />
    </p>
    
    <p class="meta-options third-meta-options">
        <label for="page_header_image_full_screen">'.__('Full Screen?','wprentals').'</label><br />
    ...
    <p class="meta-options third-meta-options">
        <label for="page_header_image_back_type">'.__('Full Screen Background Type?','wprentals').'</label><br />
    ...
    <p class="meta-options third-meta-options">
        <label for="page_header_title_over_image">'.__('Title Over Image','wprentals').'</label><br />
    ...
    <p class="meta-options third-meta-options">
        <label for="page_header_subtitle_over_image">'.__('SubTitle Over Image','wprentals').'</label><br />
    ...
    <p class="meta-options third-meta-options">
        <label for="page_header_image_height">'.__('Image Height(Ex:700, Default:580px)','wprentals').'</label><br />
        ...
        <div class="meta-options third-meta-options">
            <label for="page_header_overlay_color">'.__('Overlay Color','wprentals').'</label><br />
        ...
        <p class="meta-options third-meta-options">
            <label for="page_header_overlay_val">'.__('Overlay Opacity(betwen 0 and 1 , Ex:0.5, default 0.6)','wprentals').'</label><br />
...
function estate_page_video_box($post) {
    ...
    <div class="header_admin_options video_header">
        <p class="meta-options pblank">
            <h3 class="pblankh">'.__('Options for Video Header','wprentals').'</h3>
        </p>
      
 
   
        <p class="meta-options ">
            <label for="page_custom_image">'.__('Video MP4 version','wprentals').'</label><br />
            <input id="page_custom_video" type="text" size="36" name="page_custom_video" value="'.$page_custom_video.'" />
            <input id="page_custom_video_button" type="button"   size="40" class="upload_button button" value="'.__('Upload Video','wprentals').'" />
        </p>
        
        <p class="meta-options ">
            <label for="page_custom_image">'.__('Video WEBM version','wprentals').'</label><br />
            <input id="page_custom_video_webbm" type="text" size="36" name="page_custom_video_webbm" value="'.$page_custom_video_webbm.'" />
            <input id="page_custom_video_webbm_button" type="button"   size="40" class="upload_button button" value="'.__('Upload Video','wprentals').'" />
        </p>
        
        <p class="meta-options ">
            <label for="page_custom_image">'.__('Video OGV version','wprentals').'</label><br />
            <input id="page_custom_video_ogv" type="text" size="36" name="page_custom_video_ogv" value="'.$page_custom_video_ogv.'" />
            <input id="page_custom_video_ogv_button" type="button"   size="40" class="upload_button button" value="'.__('Upload Video','wprentals').'" />
        </p>
        
        <p class="meta-options ">
            <label for="page_custom_video_cover_image">'.__('Cover Image','wprentals').'</label><br />
            ...
            <input id="page_custom_video_cover_image_button" type="button"   size="40" class="upload_button button" value="'.__('Upload Image','wprentals').'" />
        </p>
        
        <p class="meta-options third-meta-options">
            <label for="page_header_video_full_screen">'.__('Full Screen?','wprentals').'</label><br />
        ...
        <p class="meta-options third-meta-options">
            <label for="page_header_title_over_video">'.__('Title Over Image','wprentals').'</label><br />
        ...
        <p class="meta-options third-meta-options">
            <label for="page_header_subtitle_over_video">'.__('SubTitle Over Image','wprentals').'</label><br />
        ...
        <p class="meta-options third-meta-options">
            <label for="page_header_video_height">'.__('Video Height(Ex:700, Default:580px)','wprentals').'</label><br />
            <input id="page_header_video_height" type="text" size="36" name="page_header_video_height" value="'.$page_header_video_height.'" />
        </p>    

        <div class="meta-options third-meta-options">
            <label for="page_header_overlay_color_video">'.__('Overlay Color','wprentals').'</label><br />
        ...
        <p class="meta-options third-meta-options">
            <label for="page_header_overlay_val_video">'.__('Overlay Opacity(betwen 0 and 1 , Ex:0.5, default 0.6)','wprentals').'</label><br />
...
function estate_page_theme_slider($post) {
    ...
    <div class="header_admin_options theme_slider">
        <p class="meta-options pblank">
            <h3 class="pblankh">'.__('Options for Theme Slider','wprentals').'</h3>
        </p>
        <p class="meta-options">	
...
if( !function_exists('estate_page_map_box_agent') ):
    function estate_page_map_box_agent($post) {
        ...
        <p class="meta-options">
            <label for="page_custom_image">'.esc_html__( 'Replace Map with this image','wprentals').'</label><br />
            <input id="page_custom_image" type="text" size="36" name="page_custom_image" value="'.$page_custom_image.'" />
            <input id="page_custom_image_button" type="button"   size="40" class="upload_button button" value="'.esc_html__( 'Upload Image','wprentals').'" />
         </p>
...
if( !function_exists('estate_page_options_box') ):
    function estate_page_options_box($post) {
        ...
        print '
        <p class="meta-options">	
        <label for="page_show_title">'.esc_html__( 'Show Title: ','wprentals').'</label><br />
...
if( !function_exists('estate_post_options_box') ):
    function estate_post_options_box($post) {
        ...
        print   '<p class="meta-options">	
                    <label for="post_show_title">'.esc_html__( 'Show Title:','wprentals').' </label><br />
        ...
        print'  <p class="meta-options">	
                    <label for="group_pictures">'.esc_html__( 'Group pictures in slider?(*only for blog posts)','wprentals').' </label><br />
...
if( !function_exists('estate_sidebar_box') ):
    function estate_sidebar_box($post) {
        ...
        print '   
        <p class="meta-options"><label for="sidebar_option">'.esc_html__( 'Where to show the sidebar: ','wprentals').' </label><br />
        ...
        print'
        <p class="meta-options"><label for="sidebar_select">'.esc_html__( 'Select the sidebar: ','wprentals').'</label><br />                  
