<?php
  ...
  function ajax_create_print(){ 
    ...
    $title= esc_html__('Invoice','wprentals');
...
function wpestate_show_license_form(){
    ...
    if($theme_activated!='is_active'){
        ...
        print '<div class="license_check_wrapper">';
            echo' <div class="activate_notice notice_here">'.__('Please activate the theme in the next 24h to validate the purchase and continue to have access to all theme options! See this <a href="https://help.wprentals.org/article/where-is-my-purchase-code/" target="_blank">link</a> if you don\'t know how to get your license key. Thank you!','wprentals').'</div>';
...
if( !function_exists('wpestate_check_license_function') ):
    function wpestate_check_license_function(){
        ...
        curl_setopt($ch, CURLOPT_URL, "http://support.wpestate.org/theme_license_check_rentals.php");
...
function wpestate_start_demo_import(){
    ...
    echo '<li>'.__('Demo content is imported.','wprentals').'</li>';
    ...
    echo '<li>'.__('Menus are configured','wprentals').'</li>';
    ...
    if (!empty($homepage->ID)) {
        ...
        echo '<li>'.__('Default homepage is configured','wprentals').'</li>';
    }else{
        echo '<li>'.__('Default homepage is NOT configured','wprentals').'</li>';
    }
    ...
    if (!empty($data)) {
        ...
        echo '<li>'.__('Theme options are imported.','wprentals').'</li>';
    }
     ///////////////////// widgets import
    ...
    foreach ($data as $sidebar_id => $widgets) {
        ...
        if (isset($wp_registered_sidebars[ $sidebar_id ])) {
            ...
            $sidebar_message        = __('Sidebar does not exist', 'wprentals');
        }
        ...
        foreach ($widgets as $widget_instance_id => $widget) {
                ...
                if (!$fail && !isset($available_widgets[ $id_base ])) {
                        ...
                        $widget_message         = __('Site does not support this widget', 'wprentals');
                }
                ...
                if (!$fail && isset($widget_instances[ $id_base ])) {
                        ...
                        foreach ($single_widget_instances as $check_id => $check_widget) {
                            if (in_array("$id_base-$check_id", $sidebar_widgets) && (array)$widget == $check_widget) {
                                ...
                                $widget_message = __('Widget already exists', 'wprentals');
                                break;
                            }
                        }
                }
                ...
                if (!$fail) {
                        ...
                        if ($sidebar_available) {
                                ...
                                $widget_message = __('Imported', 'wprentals');
                        } 
                        else {
                                $widget_message_type = 'warning';
                                $widget_message = __('Imported to Inactive', 'wprentals');
                        }
                }
                ...
                if($widget->title){
                    ...
                    $results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title'] =__('No Title', 'wprentals');
                }
...
if( !function_exists('wpestate_direct_pay_booking') ):
    function wpestate_direct_pay_booking(){
        ...
        if (function_exists('icl_translate') ){
            ...
            $payment_details =   get_option('wp_estate_direct_payment_details','') ;
        }
...
if( !function_exists('wpestate_get_booking_data') ):
    ...
    function wpestate_get_booking_data(){
        ...
        if(get_option('wp_estate_item_rental_type',true)!=1){
...
if( !function_exists('wpestate_update_menu_bar') ):
    function wpestate_update_menu_bar(){
        ...
        if($home_url!=$dash_link){
            $menu.=' <a href="'.$dash_link.'" ><i class="fas fa-map-marker"></i>'. esc_html__( 'My Listings','wprentals').'</a>';
        }

        if($home_url!=$add_link){
            $menu.='<a href="'.$add_link.'" ><i class="fas fa-plus"></i>'. esc_html__( 'Add New Listing','wprentals').'</a>';        
        }
                            ...
                            print'
                                ...
                                <div class="">
                                    <select id="booking_guest_no"  name="booking_guest_no"  class="cd-select form-control" >
                                        ...
                                        for ($i = 2; $i <=    intval   ( get_option('wp_estate_guest_dropdown_no','') ); $i++) {
...
if( !function_exists('wpestate_ajax_show_login_form') ):
    ...
    function wpestate_ajax_show_login_form(){
        ...
        $mesaj_big  =   esc_html__( 'Playa Rental Properties','wprentals');
        $sub_mesaj  =   esc_html__( 'Please fill the login form','wprentals');
        if($login_text==2){
                                ...
                                <div id="reveal_login"><a href="#">'.esc_html__( 'Already a member? Sign in!','wprentals').'</div> ';
                                    ...
                                    if($social_register_on=='yes'){
                                        ...
                                        <div class="login-links" >';
                                            ...
                                            if($facebook_status=='yes'){
                                                print '<div id="facebooklogin_reg" data-social="facebook"><i class="fab fa-facebook"></i> '.esc_html__( 'Login with Facebook','wprentals').'</div>';
                                            }
                                            if($google_status=='yes'){
...
if( !function_exists('wpestate_ajax_google_login_new') ):
    ...
    function wpestate_ajax_google_login_new(){ 
        ...
        $redirect_uri   =   wpestate_get_template_link('user_dashboard_profile.php');
...
if( !function_exists('wpestate_ajax_agent_contact_page') ):
    ...
    function wpestate_ajax_agent_contact_page(){
        ...
        if ( isset($_POST['name']) ) {
           if( trim($_POST['name']) =='' || trim($_POST['name']) ==esc_html__( 'Your Name','wprentals') ){
               echo json_encode(array('sent'=>false, 'response'=>esc_html__( 'The name field is empty !','wprentals') ));         
               exit(); 
           }else {
        ...
        //Check email
        if ( isset($_POST['email']) || trim($_POST['email']) ==esc_html__( 'Your Email','wprentals') ) {
              if( trim($_POST['email']) ==''){
                    echo json_encode(array('sent'=>false, 'response'=>esc_html__( 'The email field is empty','wprentals' ) ) );      
                    exit(); 
              } else if( filter_var($_POST['email'],FILTER_VALIDATE_EMAIL) === false) {
                    echo json_encode(array('sent'=>false, 'response'=>esc_html__( 'The email doesn\'t look right !','wprentals') ) ); 
                    exit();
              } else {
        ...
        $subject =esc_html__( 'Contact form from ','wprentals') . esc_html( home_url() ) ;

        //Check comments 
        if ( isset($_POST['comment']) ) {
              if( trim($_POST['comment']) =='' || trim($_POST['comment']) ==esc_html__( 'Your Message','wprentals')){
                echo json_encode(array('sent'=>false, 'response'=>esc_html__( 'Your message is empty !','wprentals') ) ); 
                exit();
              }else {
        ...
        $message .= esc_html__( 'Client Name','wprentals').": " . $name . "\n\n ".esc_html__( 'Email','wprentals').": " . $email . " \n\n ".esc_html__( 'Website','wprentals').": " . $website . " \n\n ".esc_html__( 'Subject','wprentals').": " . $subject . " \n\n".esc_html__( 'Message','wprentals').": \n " . $comment;
        $message .="\n\n".esc_html__( 'Message sent from contact page','wprentals');
        $headers = 'From: No Reply <noreply@'.$_SERVER['HTTP_HOST'].'>' . "\r\n";
        ...
        if( $duplicate_email_adr!='' ){
            $message = $message.' '.__('Message was also sent to ','wprentals').$receiver_email;
            wp_mail($duplicate_email_adr, $subject, $message, $headers);
        }
       
        
        echo json_encode(array('sent'=>true, 'response'=>esc_html__( 'The message was sent !','wprentals') ) ); 
    die();         
}
...
if( !function_exists('wpestate_ajax_paypal_pack_recuring_generation') ):
    ...
    function wpestate_ajax_paypal_pack_recuring_generation(){
        ...
        if( !empty ( $is_pack ) ) {
            ...
            $obj->productdesc               =   urlencode($pack_name.esc_html__( ' package on ','wprentals').get_bloginfo('name') );
            $time                           =   time(); 
            $date                           =   date('Y-m-d H:i:s',$time); 
