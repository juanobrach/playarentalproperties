...
//error_log("Appendix:" . json_encode($transient_appendix));
if($prop_selection==false){
    if( $order==0 ){
        add_filter( 'posts_orderby', 'wpestate_my_order' );
        $prop_selection = new WP_Query($args);
		//error_log("SelectionFalse:" . json_encode($args));
        remove_filter( 'posts_orderby', 'wpestate_my_order' );
    }else{
        $prop_selection = new WP_Query($args);
		//error_log("SelectionTrue:" . json_encode($args));
    }
    
    set_transient(  'wpestate_prop_list'.$transient_appendix, $prop_selection, 60*4*4 );
	//error_log("SetTransient:" . json_encode($prop_selection));
}
